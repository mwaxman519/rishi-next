...
export default function ResponsiveLayout({ children }: ResponsiveLayoutProps) {
  ...
  // During server render or initial client render, return a minimal placeholder
  // that's identical on both server and client to prevent hydration mismatches
  if (!mounted) {
    return <ServerPlaceholder>{children}</ServerPlaceholder>;
  }

  // After hydration, show a loading state while waiting for authentication and data
  if (isLoading) {
    return <PageLoader />;
  }
