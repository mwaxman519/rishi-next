...
export default function SidebarLayout({ children }: SidebarLayoutProps) {
  ...
  if (userRole) {
    ...
    console.log("No valid role found, using fallback navigation");
    // Fallback to field manager navigation if no role is specified
    roleBasedNavigation = getNavigationForRole("internal_field_manager");
  }
                  ...
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      ...
                      return fieldManagerNav.map((item, index) => {
                        ...
                        return (
                          <li key={`field-manager-${index}-${item.label}`}>
                            <Link
                              href={itemPath}
                    ...
                    {(() => {
                      ...
                      return brandAgentNav.map((item, index) => {
                        ...
                        const isActive =
                          ...
                          (itemPath !== "/" && pathname?.startsWith(itemPath));
                        return (
                          <li key={`brand-agent-${index}-${item.label}`}>
                            <Link
                              href={itemPath}
                              className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                isActive
                  ...
                  <ul className="pl-4 space-y-1">
                    {(() => {
                      ...
                      return clientUserNav.map((item, index) => {
                        ...
                        return (
                          <li key={`client-user-${index}-${item.label}`}>
                            <Link
                              href={itemPath}
                              className={`flex items-center px-4 py-2 rounded-md transition-colors ${
                                isActive
