...
export function useAuthService(): AuthServiceClient {
  ...
  async function authRequest<T>(
    ...
    try {
      ...
      const response = await fetch(
        `/api/auth-service/routes/${endpoint}`,
        options,
      );
  ...
  async function login(credentials: LoginCredentials): Promise<UserSession> {
    try {
      ...
      // In development mode, simulate successful login with mock data
  ...
  async function register(data: RegisterData): Promise<UserSession> {
    try {
      ...
      // In development mode, simulate successful registration with mock data
      if (process.env.NODE_ENV === "development") {
        ...
        return {
          ...
          roles: [data.role?.toUpperCase() || "BRAND_AGENT"],
        };
      }
  ...
  async function logout(): Promise<void> {
    try {
      // In development mode, simulate successful logout
      if (process.env.NODE_ENV === "development") {
        console.log("DEVELOPMENT MODE: Simulating successful logout");
        await new Promise((resolve) => setTimeout(resolve, 300));

        // For development, redirect to page with unauthenticated parameter
      ...
      // Redirect to login page after logout in production
      if (typeof window !== "undefined") {
  ...
  async function getSession(): Promise<SessionInfo> {
    try {
      // In development mode, use mock data and don't make HTTP requests to avoid NextAuth conflicts
      if (process.env.NODE_ENV === "development") {
        const urlParams =
        ...
        if (showUnauthenticated) {
          console.log("DEVELOPMENT MODE: Showing unauthenticated state");
          return { user: null };
        }

        // Return mock authenticated user without HTTP request to prevent NextAuth conflicts
        console.log(
          "DEVELOPMENT MODE: Using mock user for session (no HTTP request)",
        );
      ...
      // For production, call the real session endpoint
