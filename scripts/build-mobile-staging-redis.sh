...
# Build Mobile Staging Package with Dual Redis Architecture
# Creates VoltBuilder-ready package for Android/iOS compilation

set -e

echo "🏗️  Building Rishi Platform Mobile - Staging (with Redis Event Coordination)"
echo "=================================================="

# Clean previous builds
echo "🧹 Cleaning previous builds..."
rm -rf rishi-mobile-staging-redis-*.zip
rm -rf mobile-staging-redis-build/

# Create build directory
mkdir -p mobile-staging-redis-build
cd mobile-staging-redis-build
...
echo "🔧 Configuring for staging environment with Redis..."

# Create staging environment file
cat > .env.staging << 'EOF'
# Staging Mobile Environment
NODE_ENV=staging
NEXT_PUBLIC_APP_ENV=staging

# Staging Database
DATABASE_URL=postgresql://neondb_owner:npg_tuJ0c2Lqwzmg@ep-blue-forest-a4kz5fpa.pooler.neon.tech/rishiapp_staging?sslmode=require

# Staging Redis (Replit Redis Cloud - Key Prefix Isolation)
...
NEXT_PUBLIC_VERCEL_URL=https://rishi-staging.replit.app
NEXT_PUBLIC_API_URL=https://rishi-staging.replit.app/api
...
const nextConfig = {
  ...
  generateBuildId: () => 'staging-mobile-build',
...
# Update Capacitor config for staging
...
const config: CapacitorConfig = {
  ...
  webDir: 'out',
  server: {
    // Hybrid approach: Static files + Live API
...
android {
    namespace 'com.rishiplatform.staging'
    ...
    defaultConfig {
        applicationId "com.rishiplatform.staging"
...
PACKAGE_NAME="rishi-mobile-staging-redis-$(date +%Y%m%d-%H%M).zip"
zip -r "$PACKAGE_NAME" mobile-staging-redis-build/ \
  -x "mobile-staging-redis-build/node_modules/*" \
  -x "mobile-staging-redis-build/.next/*" \
  -x "mobile-staging-redis-build/.git/*" \
  -x "*.DS_Store" \
  -x "*.log"
...
echo ""
echo "✅ Mobile staging package created successfully!"
echo "📦 Package: $PACKAGE_NAME"
echo "📏 Size: $PACKAGE_SIZE"
echo ""
echo "🏗️  Redis Architecture Configuration:"
echo "   • Environment: staging"
echo "   • Redis Provider: Replit Redis Cloud"
echo "   • Key Prefix: events:staging:*"
echo "   • Database: 0 (shared with development)"
...
echo "   • Redis Events: Enabled with staging prefix"
echo ""
...
echo "🎉 Staging mobile build with Redis event coordination complete!"